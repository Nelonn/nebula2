	...
	assert.Equal(
		...
		"Usage of "+os.Args[0]+" ca <flags>: create a self signed certificate authority\n"+
			...
			"    \tOptional: output a qr code image (png) of the certificate\n"+
				...
				"  -version uint\n"+
				"    \tOptional: version of the certificate format to use (default 3)\n",
		ob.String(),
	)
}
...
func Test_ca(t *testing.T) {
	...
	assertHelpError(t, ca(
		[]string{"-version", "3", "-out-key", "nope", "-out-crt", "nope", "duration", "100m"}, ob, eb, nopw,
	), "-name is required")
	...
	ob.Reset()
	eb.Reset()
	args := []string{"-version", "3", "-name", "test", "-duration", "100m", "-out-crt", "/do/not/write/pleasecrt", "-out-key", "/do/not/write/pleasekey"}
	...
	ob.Reset()
	eb.Reset()
	args = []string{"-version", "3", "-name", "test", "-duration", "100m", "-out-crt", "/do/not/write/pleasecrt", "-out-key", keyF.Name()}
	...
	ob.Reset()
	eb.Reset()
	args = []string{"-version", "3", "-name", "test", "-duration", "100m", "-groups", "1,,   2    ,        ,,,3,4,5", "-out-crt", crtF.Name(), "-out-key", keyF.Name()}
	...
	ob.Reset()
	eb.Reset()
	args = []string{"-version", "3", "-encrypt", "-name", "test", "-duration", "100m", "-groups", "1,2,3,4,5", "-out-crt", crtF.Name(), "-out-key", keyF.Name()}
